...
import { join, isAbsolute, basename, extname, resolve } from 'path';
...
function parseSimpleYaml(yaml: string): Record<string, unknown> {
  ...
  const stack: Array<{ type: 'object' | 'array'; data: any; key: string | null; indent: number }> = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    let ctx: Record<string, unknown> = result;
    for (const frame of stack) {
      if (frame.type === 'array') {
        ctx = (frame.data as any[])[frame.data.length - 1] as Record<string, unknown>;
      } else {
        ctx = frame.data as Record<string, unknown>;
      }
    }
    ...
    if (arrayMatch) {
      ...
      if (inlineObjMatch && !value.startsWith('"') && !value.startsWith("'")) {
        ...
        (targetCtx[arrayKey] as unknown[]).push(obj);
        stack.push({ type: 'array', data: targetCtx[arrayKey], key: arrayKey, indent: lineIndent });
      } else {
        // Simple value
        (targetCtx[arrayKey] as unknown[]).push(parseYamlValue(value));
      }
      continue;
    }
    ...
    if (kvMatch) {
      ...
      if (stack.length > 0 && stack[stack.length - 1].type === 'array' && lineIndent > stack[stack.length - 1].indent) {
        const arr = stack[stack.length - 1].data as any[];
...
class MigrationService {
  ...
  private resolvePath(relativeOrAbsolute: string): string {
    if (this.isAbsolutePath(relativeOrAbsolute)) {
      return relativeOrAbsolute;
    }
    return resolve(this.options.vault, relativeOrAbsolute);
  }
  ...
  private listMarkdownFiles(dirPath: string): Array<{ content: string; fileName: string; fullPath: string }> {
    const result: Array<{ content: string; fileName: string; fullPath: string }> = [];
  ...
  async run(): Promise<boolean> {
    ...
    // 检查旧数据是否存在
    const hasOldData = existsSync(oldDataPath);
    const hasTodolist = existsSync(todolistPath);
  ...
  private async backupOriginalData(
    oldDataPath: string,
    todolistPath: string,
    newDataPath: string,
    timestamp: number,
  ): Promise<void> {
    const backupDir = join(newDataPath, `_migration_backup_${timestamp}`);
    ...
    if (existsSync(oldDataPath)) {
      const avmBackupDir = join(backupDir, 'avm');
    ...
    if (existsSync(todolistPath)) {
      const todolistBackupDir = join(backupDir, 'todolist');
      this.ensureDir(todolistBackupDir);
      copyFileSync(todolistPath, join(todolistBackupDir, 'tasks.json'));
  ...
  private async migrateAVMEntities(oldDataPath: string, newDataPath: string): Promise<void> {
    this.log('info', '--- 迁移 AVM 实体 ---');

    // 迁移 Apps
    const oldAppsDir = join(oldDataPath, 'apps');
    const newAppsDir = join(newDataPath, 'apps');
    if (existsSync(oldAppsDir)) {
      ...
      for (const file of files) {
        ...
        const filePath = join(newAppsDir, fileName);
        this.writeFile(filePath, createFrontmatter(app as unknown as Record<string, unknown>));
        this.stats.avmApps++;
      }
    ...
    const versionToAppMap = new Map<string, string>();
    const oldVersionsDir = join(oldDataPath, 'versions');
    const newVersionsDir = join(newDataPath, 'versions');
    if (existsSync(oldVersionsDir)) {
      ...
      for (const file of files) {
        ...
        const filePath = join(newVersionsDir, fileName);
        this.writeFile(filePath, createFrontmatter(version as unknown as Record<string, unknown>));
    ...
    const oldProjectsDir = join(oldDataPath, 'projects');
    const newProjectsDir = join(newDataPath, 'projects');
    if (existsSync(oldProjectsDir)) {
      const files = this.listMarkdownFiles(oldProjectsDir);
      for (const file of files) {
        // 跳过 todos__*.md（待办单独处理）
        if (file.fileName.startsWith('todos__')) continue;
        ...
        const filePath = join(newProjectsDir, fileName);
  ...
  private async migrateAVMTodos(oldDataPath: string, newDataPath: string): Promise<void> {
    this.log('info', '--- 迁移 AVM 待办 ---');

    const oldProjectsDir = join(oldDataPath, 'projects');
    const newTodosDir = join(newDataPath, 'todos');

    if (!existsSync(oldProjectsDir)) return;
    ...
    for (const file of files) {
      ...
      for (const todoRaw of todosRaw) {
        if (typeof todoRaw !== 'object' || todoRaw === null) continue;
        const todoFm = todoRaw as Record<string, unknown>;
        ...
        const filePath = join(newTodosDir, fileName);
        this.writeFile(filePath, createFrontmatter(todo as unknown as Record<string, unknown>));
        this.stats.avmTodos++;
      }
    }
  ...
  private async migrateTodolist(todolistPath: string, newDataPath: string): Promise<void> {
    ...
    for (const dayGroup of data.tasks) {
      if (!dayGroup.tasksList || !Array.isArray(dayGroup.tasksList)) continue;
  ...
  private convertApp(fm: Record<string, unknown>): App {
    return {
  ...
  private convertVersion(fm: Record<string, unknown>): Version | null {
    if (!fm.id || !fm.appId) return null;
    return {
      id: String(fm.id),
  ...
  private convertProject(fm: Record<string, unknown>, versionToAppMap?: Map<string, string>): Project {
    // 解析 progressHistory
    const progressHistory: ProgressHistoryItem[] = [];
    ...
    const appVersionLinks: ProjectLink[] = [];
    if (Array.isArray(fm.appVersionLinks)) {
      ...
      const vid = fm.versionId as string;
  ...
  private projectToFrontmatter(project: Project): Record<string, unknown> {
    return {
      ...project,
  ...
  private convertAVMTodo(fm: Record<string, unknown>, projectId: string | null): Todo {
  ...
  private convertTodolistTask(task: Record<string, unknown>): Todo {
    ...
    return {
      ...
      status: completed ? 'done' : 'todo',
      pinned: false, // todolist 无置顶概念
      categoryId: null, // todolist 无分类概念
      projectId: null, // todolist 无项目概念
      responsiblePerson: '', // todolist 无负责人概念
      completedAt: completed ? createdAt : '',
      createdAt,
      updatedAt: createdAt, // todolist 无 updatedAt
      version: 1,
    };
  }
  ...
  private inferAppNameFromFile(fileName: string, fm: Record<string, unknown>): string {
    // 尝试从 frontmatter 获取 appName
    if (fm.appName && typeof fm.appName === 'string') return fm.appName;
  ...
  private async writeMigrationLog(newDataPath: string, timestamp: number): Promise<void> {
    const logPath = join(newDataPath, `_migration_${timestamp}.log`);
  ...
  private verify(): boolean {
    ...
    this.log(
      'info',
      `统计: ${this.stats.avmApps} apps, ${this.stats.avmVersions} versions, ${this.stats.avmProjects} projects, ${totalTodos} todos`,
    );
