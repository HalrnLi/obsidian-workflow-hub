...
结论：**项目能编译，架构设计清晰，但存在 1 个高危数据丢失 bug、多处安全/数据完整性隐患，以及若干工程问题。**
...
### 架构设计
- **分层清晰**：Service 层（DataService/TodoService/CategoryService/...）与 View 层（AppVersionManagerView/DualPaneView/TableView/TodoTabView）职责分离明确
- **数据存储设计合理**：Markdown + YAML frontmatter，Obsidian 原生可读可搜索；每个实体独立文件（`{name}__{id}.md`），避免大文件读写
- **内存索引设计**（TodoService）：多维索引（byId/byCategory/byProject/byStatus/byDueDate/searchIndex），CRUD 时增量维护，中文 2-gram 分词搜索
- **数据迁移方案**：备份 → 迁移 → 自检 → 标记完成的流程设计，迁移日志持久化
...
### 10. `MigrationService` 只支持 vault 相对路径的迁移
- 位置：`src/services/MigrationService.ts:109`，仅当 `!this.isAbsolutePath(this.oldDataPath)` 时才备份和迁移。
- 影响：若用户之前用绝对路径存储 AVM 数据，迁移不会执行，造成数据孤岛。
- 修复：迁移服务应同时支持绝对路径旧数据的检测与迁移。

### 11. `BackupService` 不支持绝对路径
- `BackupService` 完全依赖 Obsidian vault API（`this.app.vault.createFolder`、`this.app.vault.create`），如果用户设置了绝对路径的数据存储，备份功能会失败或写入错误位置。

### 12. `CategoryService.delete` 不级联处理待办
- 删除分类后，原有待办的 `categoryId` 仍指向已删除的分类 ID，导致 UI 显示异常。
- 修复：删除分类时将其下待办的 `categoryId` 置为 `null`。

### 13. `settings.migrationCompleted` 的重试机制缺陷
- 如果迁移失败（如中途报错），`migrationCompleted` 仍为 `false`，下次启动会重新尝试。但此时部分数据可能已经迁移成功，会导致重复数据。
- 修复：迁移前记录已迁移的实体 ID，重试时跳过。

---

## 🟡 低危 / 工程

### 14. `npm run version` 脚本引用不存在的文件
...
- `DataCache` 无大小限制，长期使用内存会持续增长；TTL 只在 `get` 时检查，过期条目不访问就不清理，造成内存泄漏。

### 18. `createFrontmatter` 传入非数组对象值会写成 `[object Object]`
- 例如 `upsertAppRecord(record as unknown as Record<string, unknown>)`，若 record 含嵌套非数组对象，序列化后变成字符串 `[object Object]`，回读即损坏。

### 19. `DataService` 大量重复代码
- `createApp/updateApp/deleteApp`、`createVersion/updateVersion/deleteVersion`、`createProject/updateProject/deleteProject`、`createPlan/updatePlan/deletePlan` 的代码模式高度重复（~1000 行）。
- 建议抽一个通用的 `EntityService<T>` 基类，或引入 `FileAdapter` 接口消除绝对/相对路径的分支散落。

### 20. 类型安全
- `types.ts` 中大量使用 `any` 类型（如 `parseFrontmatter` 返回 `Record<string, any>`）
- `MigrationService` 中 `convertAVMTodo` 和 `convertTodolistTask` 参数是 `any`
- `(project as any)[prevKey]` 和 `(project as any)[stage.key]` 的类型断言

### 21. `types.ts` 文件过长（464 行）
- 混合了类型定义、常量、业务逻辑函数。建议：
  - 类型定义放 `types.ts`
  - 业务函数移到 `utils/projectLogic.ts` 或 `utils/dateUtils.ts`
  - `parseDateInput`（120 行）独立成一个文件

### 22. `TodoService.tokenize` 对英文数字混合词的处理
- 像 `obsidian123` 或 `utf8` 这样的词会被整体索引，但搜索时如果用户搜 `obsidian` 就匹配不到。

### 23. `AppVersionManagerView.render()` 频繁重建 DOM
- 每次调用 `render()` 都会 `this.containerEl.empty()` 然后重建整个视图。在搜索防抖（180ms）场景下，用户每输入一个字符就可能触发完整重渲染。

### 24. `registerEvents` 是空方法
- `src/view/AppVersionManagerView.ts:124`：`private registerEvents() {}`

### 25. 测试覆盖不足
- 当前只有 `DataService.test.ts` 一个测试文件。缺少：
  - `TodoService` / `CategoryService` 的索引测试
  - `MigrationService` 的迁移逻辑测试
  - `frontmatter.ts` 的解析测试
  - `types.ts` 中业务函数的单元测试
...
| 优先级 | 编号 | 说明 |
|--------|------|------|
| P0 | #1 | projectInfo 丢失 — 直接丢数据 |
| P0 | #2 | href XSS — 高危安全漏洞 |
| P1 | #3 | 备份补全 Todo/Category |
| P1 | #4 | 用 Obsidian 的 `parseYaml/stringifyYaml` 替换自研 frontmatter |
| P1 | #5 | 时间戳格式统一 |
| P1 | #8 | 升级 xlsx 到安全版本 |
| P2 | #6 | `parseFrontmatter` 空 frontmatter 返回 null |
| P2 | #7 | 删除操作原子性 |
| P2 | #12 | 分类删除级联处理待办 |
| P3 | #10, #11, #14, #15 | 工程清理项 |
